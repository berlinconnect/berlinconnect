  ...
  <section class="sundays">
    ...
    <div class="slates">
      <?php foreach($pages->find('sections')->children()->filterBy('umbrella', 'Sundays', ',') as $section): ?>
        <div id="<?= $section->uniqueid() ?>" class="slate <?php if (strlen($section->pagelink()) < 1): ?>dimmed<?php endif ?> <?= $section->slatesize() ?>">
  ...
  <section id="community" class="community">
    ...
    <div class="slates">
      <?php foreach($pages->find('sections')->children()->filterBy('umbrella','Community') as $section): ?>
        <div id="<?= $section->uniqueid() ?>" class="slate dimmed <?= $section->slatesize() ?>">
          <?php if (strlen($section->pagelink()) != 0): ?>
            <a class="pageLink" href="<?= $section->pagelink() ?>"></a>
          <?php endif ?>
          <div class="background-image" style="background-image: url(<?= $section->images()->first()->url(); ?>)"></div>
